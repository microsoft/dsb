...
static int * pCompanyIdPermutation;
...
mk_w_web_site (void *row, ds_key_t index)
{
   ...
   tdef *pT = getSimpleTdefsByNumber(WEB_SITE);
	...
	if (!bInit)
	{
		...
		pMarketIdPermutation = makePermutation(NULL, 6, WEB_MARKET_ID);
		pCompanyIdPermutation = makePermutation(NULL, 6, WEB_COMPANY_ID);
  ...
  */
	...
	// Add skews to data
	genrand_integer(&r->web_market_id, DIST_EXP_RANK_LOW, 1, 6, 0, WEB_MARKET_ID);
	mapPermutation(&r->web_market_id, pMarketIdPermutation);
	...
	// Add skews to data
	genrand_integer(&r->web_company_id, DIST_EXP_RANK_LOW, 1, 6, 0,
		WEB_COMPANY_ID);
	mapPermutation(&r->web_company_id, pCompanyIdPermutation);
