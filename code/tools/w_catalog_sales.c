...
static ds_key_t * pCallCenterPermutation;
...
mk_master (void *row, ds_key_t index)
{
	...
	if (!bInit)
	{
		...
		pCustomerPermutation = makeKeyPermutation(NULL, get_rowcount(CUSTOMER), CS_BILL_CUSTOMER_SK);
		pShipModePermutation = makeKeyPermutation(NULL, get_rowcount(SHIP_MODE), CS_SHIP_MODE_SK);
		pPromotionPermutation = makeKeyPermutation(NULL, get_rowcount(PROMOTION), CS_PROMO_SK);
		pCallCenterPermutation = makeKeyPermutation(NULL, get_rowcount(CALL_CENTER), CS_CALL_CENTER_SK);
		pWarehousePermutation = makeKeyPermutation(NULL, get_rowcount(WAREHOUSE), CS_WAREHOUSE_SK);
		...
		pWholesaleCostPermutation = makePermutation(NULL, WHOLESALE_COST_BUCKET_COUNT, CATALOG_SALES);
		pMarkupPermutation = makePermutation(NULL, MARKUP_BUCKET_COUNT, CATALOG_SALES);
		pDiscountPermutation = makePermutation(NULL, DISCOUNT_BUCKET_COUNT, CATALOG_SALES);

		// Load data for joint distribution.
		pItemCategoryMap = deserializeAttributeKeyMap("item_category");
		pItemCategoryPermutation = makePermutation(NULL, pItemCategoryMap->attributeCount, CS_SOLD_ITEM_SK);
		pCustomerCountyMap = deserializeAttributeKeyMap("customer_county");
		pCustomerCountyPermutation = makePermutation(NULL, pCustomerCountyMap->attributeCount, CS_BILL_CUSTOMER_SK);
		pCustomerStateMap = deserializeAttributeKeyMap("customer_state");
		pCustomerStatePermutation = makePermutation(NULL, pCustomerStateMap->attributeCount, CS_BILL_CUSTOMER_SK);
   ...
   {
      jDate += 1;
      kNewDateIndex += dateScalingPermutation(CATALOG_SALES, jDate, pDatePermutation);
   }
    ...
    * recent values that were used to set the values of the orderline-invariant columns
 	 */
		...
		r->cs_sold_time_sk = mk_join (CS_SOLD_TIME_SK, TIME,
			r->cs_call_center_sk);
		r->cs_call_center_sk =
			(r->cs_sold_date_sk == -1)?-1:mk_join_permutation (CS_CALL_CENTER_SK, CALL_CENTER, r->cs_sold_date_sk, pCallCenterPermutation);
		...
		genrandExpJointDistribution(pItemCategoryMap, pCustomerStateMap,
			...
			CS_BILL_CUSTOMER_SK,
			&tmp, &r->cs_bill_customer_sk);
		nTicketItemBase = (int)tmp;
...
mk_detail(void *row, int bPrint)
{
   ...
   r->cs_sold_item_sk = matchSCDSK(kItem, r->cs_sold_date_sk, ITEM);
	...
	r->cs_ship_mode_sk = mk_join_permutation (CS_SHIP_MODE_SK, SHIP_MODE, 1, pShipModePermutation);
	r->cs_warehouse_sk = mk_join_permutation (CS_WAREHOUSE_SK, WAREHOUSE, 1, pWarehousePermutation);
	r->cs_promo_sk = mk_join_permutation (CS_PROMO_SK, PROMOTION, 1, pPromotionPermutation);

  // Add skews to data
	set_pricing_permutation(WS_PRICING, &r->cs_pricing, pWholesaleCostPermutation, WHOLESALE_COST_BUCKET_COUNT,
