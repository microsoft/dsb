...
AddGuassianDist(distindex_t * t)
{
	...
	for (i = 0; i < t->nDistCount; ++i) {
		...
		dist_t * dist = t->pEntries[i].dist;
		int width = t->pEntries[i].w_width;
		// Resize the weight set.
		dist->weight_sets = (int **)realloc(dist->weight_sets,
			sizeof(int *) * (width + 1));
		if (dist->weight_sets == NULL)
			return (QERR_NO_MEMORY);
		// Add new weight based on normal distribution.
		dist->weight_sets[width] = genrand_gaussian_dist(t->pEntries[i].length, get_float("PARAM_SIGMA"), get_float("PARAM_CENTER"), 0);
		// Update weight count
		t->pEntries[i].w_width += 1;
	}
...
int main(int argc, char* argv[])
{
	...
	if (is_set("VERBOSE"))
	{
		print_dist_index(pDistIndex);
	}

	if (is_set("PARAM_DIST") && strcasecmp(get_str("PARAM_DIST"), "normal") == 0)
	{
		AddGuassianDist(pDistIndex);
	}
