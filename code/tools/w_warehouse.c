...
mk_w_warehouse (void* row, ds_key_t index)
{
   ...
   tdef *pT = getSimpleTdefsByNumber(WAREHOUSE);
	...
	// Add skews to data
	r->w_warehouse_sq_ft =
		genrand_integer(NULL, DIST_EXP_RANK_LOW,
			1, sqFtBucketCount, 0, W_WAREHOUSE_SQ_FT);
	mapPermutation(&r->w_warehouse_sq_ft, pSqFtPermutation);
	makeRangeFromBucket(&rangeMin, &rangeMax, sqFtBucket, r->w_warehouse_sq_ft, W_SQFT_MIN, W_SQFT_MAX);
	genrand_integer(&r->w_warehouse_sq_ft, DIST_UNIFORM, rangeMin, rangeMax, 0, W_WAREHOUSE_SQ_FT);
