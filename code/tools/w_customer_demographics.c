...
mk_w_customer_demographics (void* row, ds_key_t index)
{
   ...
   tdef *pTdef = getSimpleTdefsByNumber(CUSTOMER_DEMOGRAPHICS);
	...
	// Update information to populate joint distribution.
	int len1 = strlen(r->cd_gender);
	int len2 = strlen(r->cd_education_status);
	int len3 = strlen(r->cd_marital_status);
	char * value =  (char *)malloc((len1 + len2 + len3 + 1) * sizeof(char));
	MALLOC_CHECK(value);
