...
static int * pClassPermutation;
static int * pHoursPermutation;
static int * pMktIdPermutation;
static int * pDivisionPermutation;
static int * pCompanyPermutation;

static int * pSqFtPermutation;
static int sqFtBucket;
static int sqFtBucketCount;

static int * pEmployeesPermutation;
static int employeesBucket;
static int employeesBucketCount;
...
mk_w_call_center (void* row, ds_key_t index)
{
   ...
   static int employeesMax, rangeMin, rangeMax;
    ...
    /* begin locals allocation/initialization */
		...
		pClassPermutation = makePermutation(NULL, distsize("call_center_class"), CC_CLASS);
		pHoursPermutation = makePermutation(NULL, distsize("call_center_hours"), CC_HOURS);
		pMktIdPermutation = makePermutation(NULL, 6, CC_MARKET_ID);
		pDivisionPermutation = makePermutation(NULL, 6, CC_COMPANY);
		pCompanyPermutation = makePermutation(NULL, 6, CC_COMPANY);

		makeBucket(&sqFtBucket, &sqFtBucketCount, 20, 700 - 100 + 1);
		pSqFtPermutation = makePermutation(NULL, sqFtBucketCount, CC_SQ_FT);

		employeesMax = CC_EMPLOYEE_MAX * nScale * nScale;
		makeBucket(&employeesBucket, &employeesBucketCount, 20, employeesMax);
		pEmployeesPermutation = makePermutation(NULL, employeesBucketCount, CC_EMPLOYEES);
 ...
 */
  ...
  */
	...
	// Add skews to data
	pick_distribution_permutation(&r->cc_class, "call_center_class", 1, DIST_EXP_RANK_LOW, CC_CLASS, pClassPermutation);
	changeSCD(SCD_PTR, &r->cc_class, &rOldValues->cc_class,  &nFieldChangeFlags,  bFirstRecord);

	// Add skews to data
	genrand_integer(&r->cc_employees, DIST_EXP_RANK_LOW, 1, employeesBucketCount, 0, CC_EMPLOYEES);
	mapPermutation(&r->cc_employees, pEmployeesPermutation);
	makeRangeFromBucket(&rangeMin, &rangeMax, employeesBucket, r->cc_employees, 1, employeesMax);
	genrand_integer(&r->cc_employees, DIST_UNIFORM, rangeMin, rangeMax, 0, CC_EMPLOYEES);

	changeSCD(SCD_INT, &r->cc_employees, &rOldValues->cc_employees,  &nFieldChangeFlags,  bFirstRecord);

	// Add skews to data
	genrand_integer(&r->cc_sq_ft, DIST_EXP_RANK_LOW, 1, sqFtBucketCount, 0, CC_SQ_FT);
	mapPermutation(&r->cc_sq_ft, pSqFtPermutation);
	makeRangeFromBucket(&rangeMin, &rangeMax, sqFtBucket, r->cc_sq_ft, 100, 700);
	genrand_integer(&r->cc_sq_ft, DIST_UNIFORM, rangeMin, rangeMax, 0, CC_SQ_FT);

	r->cc_sq_ft *= r->cc_employees;
	changeSCD(SCD_INT, &r->cc_sq_ft, &rOldValues->cc_sq_ft,  &nFieldChangeFlags,  bFirstRecord);

	// Add skews to data
	pick_distribution_permutation(&r->cc_hours, "call_center_hours",
		1, DIST_EXP_RANK_LOW, CC_HOURS, pHoursPermutation);
	...
	// Add skews to data
	genrand_integer(&r->cc_market_id, DIST_EXP_RANK_LOW, 1, 6, 0, CC_MARKET_ID);
	mapPermutation(&r->cc_market_id, pMktIdPermutation);
	...
	// Add skews to data
	genrand_integer(&r->cc_company, DIST_EXP_RANK_LOW, 1, 6, 0, CC_COMPANY);
	mapPermutation(&r->cc_company, pCompanyPermutation);

	changeSCD(SCD_INT, &r->cc_company, &rOldValues->cc_company,  &nFieldChangeFlags,  bFirstRecord);

	// Add skews to data
	genrand_integer(&r->cc_division_id, DIST_EXP_RANK_LOW, 1, 6, 0, CC_COMPANY);
	mapPermutation(&r->cc_division_id, pDivisionPermutation);
