...
static int * pYearPermutation = NULL;
static int * pMonthPermutation = NULL;
static int * pDayPermutation = NULL;

static int * pDecimalPermutation = NULL;
static int decimalBucketCount = 0;
...
/*
 * Generate a random rank in the given range with an approximate Zipfian distribution.
 * Note this returns a rank of the element, and it should be used on top of a permutation
 * of the actual elements.
 * Exponential distribution: https://en.wikipedia.org/wiki/Exponential_distribution
 * CDF: f(x) = 1 - e^(-lambda * x)
 * Fix x to be between 0 and 20.
 * x = -ln(1 - f(x)) / lambda
...
genrand_exp_rank(long long v_min, long long v_max, double lambda, int stream)
{
	...
	// Scale the probability into the range.
	double cdf_min = 1 - exp(-lambda * DIST_EXP_X_MIN);
	double cdf_max = 1 - exp(-lambda * DIST_EXP_X_MAX);
	double cdf = cdf_min + (cdf_max - cdf_min) * prob;
	// Find the x corresponding to this cdf.
...
/*
* Given a permutation of ranks, compute the CDF of the original rank in ascending order.
* For example, if the permutation is [2, 1] and the probability is [0.3, 0.7],
* remap the cdf to [0, 0.7, 1].
* Note the cdf uses 1-based indexing.
*/
double *
...
/*
* Return the normalized CDF of this rank.
* Exponential distribution: https://en.wikipedia.org/wiki/Exponential_distribution
* CDF: f(x) = 1 - e^(-lambda * x)
* Fix x to be between 0 and 20.
* x = -ln(1 - f(x)) / lambda
* Note the returned value is [0, 1]
*/
double
get_exp_cdf(long long rank, long long count, double lambda)
{
	double prob = (double)rank / count;
	double x = DIST_EXP_X_MIN + (DIST_EXP_X_MAX - DIST_EXP_X_MIN) * prob;
	double cdf = 1 - exp(-lambda * x);
	double cdf_min = 1 - exp(-lambda * DIST_EXP_X_MIN);
	double cdf_max = 1 - exp(-lambda * DIST_EXP_X_MAX);
...
/*
* Get the weight of an exponential distribution given its rank.
*/
double
get_exp_rank_weight(long long rank, long long count, double lambda)
{
	return get_exp_cdf(rank, count, lambda) - get_exp_cdf(rank - 1, count, lambda);
}

/*
* Generate discrete Gaussian distribution for n items with (0, sigma) and center.
* Center is between [-0.5, 0.5].
*/
int *
genrand_gaussian_dist(int n, double sigma, double center, int stream)
{
	// Get discrete Guassian distribution.
	double * prob = genrand_gaussian_probability(n, sigma);
	// Add a random permutation.
	int * permutation = makePermutation(NULL, n, stream);
	// Scale the probrability to integers
	int i;
	int * weight = (int *)malloc(n * sizeof(int));
	MALLOC_CHECK(weight);
	int shift = (int)(n * center) + n;
	for (i = 0; i < n; ++i) {
		weight[i] = (int)(prob[permutation[(i + shift) % n]] * 1000000);
	}
	return weight;
}

/*
* Generate a vector of probabilities for Gaussian distribution.
* Gaussian distribution: 1 / (sigma * sqrt(2*PI)) * e^(-1/2 (x-u)/sigma^2)
*/
double *
genrand_gaussian_probability(int n, double sigma)
{
	double * prob = (double *)malloc(n * sizeof(double));
	MALLOC_CHECK(prob);

	// Sample N points of (0, sigma), from -3*sigma and 3*sigma. Then normalize the probabilities to 1.
	double delta = sigma * 6 / n;
	double cur = -3 * sigma;
	double s = 0;
	int i;
	for (i = 0; i < n; ++i) {
		prob[i] = 1 / (sigma * sqrt(2 * PI)) * pow(E, -0.5 * (cur / sigma) * (cur / sigma));
		s += prob[i];
		cur += delta;
	}
...
/*
* Generate a key with 1-based index with hierarchically.
* The key is composed by [key_1, key_2, key_3, ..., etc], where each key field has a fixed
* cardinality.
* The function draws each key from a given distribution with permutation, and then
* multiple all the keys to get the final key.
*/
ds_key_t
genrand_hierarchical_key(ds_key_t * dest, int dist, int count, int * sizes, int stream, int ** permutations)
{
	ds_key_t tmp = 0;
	int value;
	for (int i = 0; i < count; ++i)
	{
		genrand_integer(&value, dist, 1, sizes[i], 0, stream);
		mapPermutation(&value, permutations[i]);
		tmp += value - 1;
		if (i < count - 1)
		{
			tmp *= sizes[i + 1];
...
/*
* Generate a permutation of decimal numbers with bucketization.
*/
int
genrand_decimal_permutation(decimal_t * dest, int dist, decimal_t * min, decimal_t * max,
	decimal_t * mean, int stream, int * permutation, int bucketCount)
{
	pDecimalPermutation = permutation;
	decimalBucketCount = bucketCount;
	int result = genrand_decimal(dest, dist, min, max, mean, stream);
	pDecimalPermutation = NULL;
	decimalBucketCount = 0;
	return result;
}
                 ...
                 decimal_t * mean, int stream)
{
   ...
   switch (dist)
	...
	case DIST_EXP_RANK_LOW:
	case DIST_EXP_RANK_HIGH:
		// Bucketize the range of decimals and scale it back.
	{
		int nTemp;
		genrand_integer(&nTemp, dist, 1, decimalBucketCount, 0, stream);
		if (pDecimalPermutation != NULL)
		{
			mapPermutation(&nTemp, pDecimalPermutation);
		}
		// Map the range from [1 .. nTemp] to [0 .. 1]
		res.number = min->number + (max->number - min->number) * (nTemp - 1) / (decimalBucketCount - 1);
		break;
	}
	default:
        INTERNAL ("Undefined distribution");
...
/*
* Generate a permutation of dates.
*/
int
genrand_date_permutation(date_t * dest, int dist, date_t * min, date_t * max,
	date_t * mean, int stream, int * yearPermutation, int * monthPermutation, int * dayPermutation)
{
	pYearPermutation = yearPermutation;
	pMonthPermutation = monthPermutation;
	pDayPermutation = dayPermutation;
	genrand_date(dest, dist, min, max, mean, stream);
	pYearPermutation = NULL;
	pMonthPermutation = NULL;
	pDayPermutation = NULL;
              ...
              date_t * mean, int stream)
{
   ...
   switch (dist)
	 ...
	 case DIST_EXP_RANK_HIGH:
	 {
		 // Generate the date in hierarchical way.
		 // First generate the year.
		 int rangeYear = max->year - min->year + 1;
		 genrand_integer(&dest->year, dist, 1, rangeYear, 0, stream);
		 if (pYearPermutation != NULL)
		 {
			 mapPermutation(&dest->year, pYearPermutation);
		 }
		 dest->year += min->year;
		 int monthMin = dest->year == min->year ? min->month : 1;
		 int monthMax = dest->year == max->year ? max->month : 12;
		 do {
			 genrand_integer(&dest->month, dist, 1, 12, imean, stream);
			 // After permutation, the month may fall out of ranges.
			 // Repeat the sampling until the month is within the valid range.
			 if (pMonthPermutation != NULL)
			 {
				mapPermutation(&dest->month, pMonthPermutation);
			 }
		 } while (dest->month < monthMin || dest->month > monthMax);
		 int dayMin = dest->year == min->year && dest->month == min->month ? min->day : 1;
		 int dayMax = dest->year == max->year && dest->month == max->month ? max->day : get_nday(dest->year, dest->month);
		 do {
			 genrand_integer(&dest->day, dist, 1, 31, imean, stream);
			 // After permutation, the month may fall out of ranges.
			 // Repeat the sampling until the month is within the valid range.
			 if (pDayPermutation != NULL)
			 {
				mapPermutation(&dest->day, pDayPermutation);
			 }
		 } while (dest->day < dayMin || dest->day > dayMax);
		 idt = dttoj(dest);
		 break;
	 }
     default:
