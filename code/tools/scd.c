...
ds_key_t
scd_join_permutation(int tbl, int col, ds_key_t jDate, ds_key_t * permutation)
{
	pPermutation = permutation;
	ds_key_t result = scd_join(tbl, col, jDate);
	pPermutation = NULL;
	return result;
}
...
scd_join(int tbl, int col, ds_key_t jDate)
{
	...
	genrand_key(&res, DIST_EXP_RANK_LOW, 1, kRowcount, 0, col);	/* pick the id */
