...
static ds_key_t * pShipModePermutation;
static ds_key_t * pPromotionPermutation;
static ds_key_t * pWarehousePermutation;
static ds_key_t * pCustomerPermutation;
static ds_key_t * pWebPagePermutation;
static ds_key_t * pWebSitePermutation;
...
mk_master (void *row, ds_key_t index)
{
   ...
   static int totalDays;
	...
	if (!bInit)
	{
		...
		pCustomerPermutation = makeKeyPermutation(NULL, get_rowcount(CUSTOMER), WS_BILL_CUSTOMER_SK);
		pShipModePermutation = makeKeyPermutation(NULL, get_rowcount(SHIP_MODE), WS_SHIP_MODE_SK);
		pPromotionPermutation = makeKeyPermutation(NULL, get_rowcount(PROMOTION), WS_PROMO_SK);
		pWarehousePermutation = makeKeyPermutation(NULL, get_rowcount(WAREHOUSE), WS_WAREHOUSE_SK);
		pWebSitePermutation = makeKeyPermutation(NULL, get_rowcount(WEB_SITE), WS_WEB_SITE_SK);
		pWebPagePermutation = makeKeyPermutation(NULL, get_rowcount(WEB_PAGE), WS_WEB_PAGE_SK);
		...
		pWholesaleCostPermutation = makePermutation(NULL, WHOLESALE_COST_BUCKET_COUNT, WEB_SALES);
		pMarkupPermutation = makePermutation(NULL, MARKUP_BUCKET_COUNT, WEB_SALES);
		pDiscountPermutation = makePermutation(NULL, DISCOUNT_BUCKET_COUNT, WEB_SALES);

		// Load data for joint distribution.
		pItemCategoryMap = deserializeAttributeKeyMap("item_category");
		pItemCategoryPermutation = makePermutation(NULL, pItemCategoryMap->attributeCount, WS_ITEM_SK);
		pCustomerCountyMap = deserializeAttributeKeyMap("customer_county");
		pCustomerCountyPermutation = makePermutation(NULL, pCustomerCountyMap->attributeCount, WS_BILL_CUSTOMER_SK);
		pCustomerStateMap = deserializeAttributeKeyMap("customer_state");
		pCustomerStatePermutation = makePermutation(NULL, pCustomerStateMap->attributeCount, WS_BILL_CUSTOMER_SK);
   ...
   {
      jDate += 1;
      kNewDateIndex += dateScalingPermutation(WEB_SALES, jDate, pDatePermutation);
   }

   r->ws_sold_date_sk = jDate;
   ...
   tdef *pT = getSimpleTdefsByNumber(WEB_SALES);
	...
	nullSet(&pT->kNullBitMap, WS_NULLS);
      ...
      */
      ...
      r->ws_web_page_sk = mk_join_permutation (WS_WEB_PAGE_SK, WEB_PAGE, r->ws_sold_date_sk, pWebPagePermutation);
      r->ws_web_site_sk = mk_join_permutation (WS_WEB_SITE_SK, WEB_SITE, r->ws_sold_date_sk, pWebSitePermutation);

      r->ws_ship_mode_sk = mk_join_permutation (WS_SHIP_MODE_SK, SHIP_MODE, 1, pShipModePermutation);
      r->ws_warehouse_sk = mk_join_permutation (WS_WAREHOUSE_SK, WAREHOUSE, 1, pWarehousePermutation);
      r->ws_promo_sk = mk_join_permutation (WS_PROMO_SK, PROMOTION, 1, pPromotionPermutation);

	  // Add skews to data
	  set_pricing_permutation(WS_PRICING, &r->ws_pricing, pWholesaleCostPermutation, WHOLESALE_COST_BUCKET_COUNT,
