...
static int * pCategoryPermutation;
...
hierarchy_item(int h_level, ds_key_t *id, char **name, ds_key_t kIndex)
{
	...
	switch(h_level)
	{
		case I_CATEGORY:
			// Add skews to data
			nLastCategory = pick_distribution_permutation(name, "categories", 1, DIST_EXP_RANK_LOW, h_level, pCategoryPermutation);
			*id = nLastCategory;
		...
		case I_CLASS:
			...
			// Add skews to data
			nLastClass = pick_distribution(name, szClassDistName, 1, DIST_EXP_RANK_LOW, h_level);
			nLastCategory = -1;
			*id = nLastClass;
			break;
		case I_BRAND:
