...
static int * pWebTypePermutation;
static int * pLinkCountPermutation;
static int * pImageCountPermutation;
static int * pMaxAdCountPermutation;
...
mk_w_web_page (void * row, ds_key_t index)
{
  ...
  */
   ...
   if (r->wp_access_date_sk == 0)
	...
	// Add skews to data
	pick_distribution_permutation(&r->wp_type, "web_page_use", 1, DIST_EXP_RANK_LOW, WP_TYPE, pWebTypePermutation);
	changeSCD(SCD_PTR, &r->wp_type, &rOldValues->wp_type,  &nFieldChangeFlags,  bFirstRecord);

	// Add skews to data
	genrand_integer(&r->wp_link_count, DIST_EXP_RANK_LOW, WP_LINK_MIN, WP_LINK_MAX, 0, WP_LINK_COUNT);
	r->wp_link_count -= WP_LINK_MIN - 1; // Shift to 1 based index
	mapPermutation(&r->wp_link_count, pLinkCountPermutation);
	r->wp_link_count += WP_LINK_MIN - 1; // Shift the index back

	changeSCD(SCD_INT, &r->wp_link_count, &rOldValues->wp_link_count,  &nFieldChangeFlags,  bFirstRecord);

	// Add skews to data
	genrand_integer(&r->wp_image_count, DIST_EXP_RANK_LOW, WP_IMAGE_MIN, WP_IMAGE_MAX, 0, WP_IMAGE_COUNT);
	r->wp_image_count -= WP_IMAGE_MIN - 1; // Shift to 1 based index
	mapPermutation(&r->wp_image_count, pImageCountPermutation);
	r->wp_image_count += WP_IMAGE_MIN - 1; // Shift the index back

	changeSCD(SCD_INT, &r->wp_image_count, &rOldValues->wp_image_count,  &nFieldChangeFlags,  bFirstRecord);

	// Add skews to data
	genrand_integer(&r->wp_max_ad_count, DIST_EXP_RANK_LOW, WP_AD_MIN, WP_AD_MAX, 0, WP_MAX_AD_COUNT);
	r->wp_max_ad_count -= WP_AD_MIN - 1; // Shift to 1 based index
	mapPermutation(&r->wp_max_ad_count, pMaxAdCountPermutation);
	r->wp_max_ad_count += WP_AD_MIN - 1; // Shift the index back
