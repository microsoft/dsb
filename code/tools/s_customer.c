...
mk_s_customer (void *pDest, ds_key_t kIndex)
{
  ...
  if (!bInit)
    {
      ...
      jtodt (&dt10YearsAgo, dtToday.julian - 365);
	    pPermutation = makePermutation (NULL, (int) getIDCount (CUSTOMER), S_CUST_ID);
  ...
  // Add skews to data
  pick_distribution(&r->pLocationType, "location_type", 1, DIST_EXP_RANK_LOW,
	  S_CUST_LOCATION_TYPE);

  // Add skews to data
  pick_distribution(&r->nVehicle, "vehicle_count", 1, DIST_EXP_RANK_LOW, S_CUST_VEHICLE_CNT);

  genrand_decimal (&r->dIncome, DIST_UNIFORM, &dMinIncome, &dMaxIncome, NULL,
		   S_CUST_INCOME);

  // Add skews to data
  pick_distribution(&r->pBuyPotential, "buy_potential", 1, DIST_EXP_RANK_LOW,
