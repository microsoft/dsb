...
static func_info_t arFuncInfo[MAX_FUNC + 1] =
{
	...
	{KW_SLIST, 2, EXPR_FL_TYPESHIFT, 0},
	{KW_SULIST, 2, EXPR_FL_TYPESHIFT, 0},
...
static int arFuncArgs[MAX_FUNC][MAX_ARGS + 1] =
{
	...
	/* KW_SLIST */		{EXPR_FL_TYPESHIFT, EXPR_FL_INT, 0},
	/* KW_SULIST */		{EXPR_FL_TYPESHIFT, EXPR_FL_INT, 0},
...
MakeListExpr(int nModifier, expr_t *pExpr, int nArg)
{
	...
	case KW_SLIST:
		pExpr->nValueCount = nArg;
		pExpr->nFlags |= EXPR_FL_SLIST;
		break;
	case KW_SULIST:
		pExpr->nValueCount = nArg;
		pExpr->nFlags |= EXPR_FL_SULIST;
		break;
	case KW_RANGE:
		pExpr->nValueCount = 2;
...
/*
* Swap the content of the two entries in the buffer list.
*/
void
SwapBuf(Expr_Val_t *pBuf, int p1, int p2)
{
	if (p1 != p2)
	{
		ds_key_t tmp = pBuf[p1].nValue;
		pBuf[p1].nValue = pBuf[p2].nValue;
		pBuf[p2].nValue = tmp;

		int tmp1 = pBuf[p1].bUseInt;
		pBuf[p1].bUseInt = pBuf[p2].bUseInt;
		pBuf[p2].bUseInt = tmp1;

		tmp1 = pBuf[p1].nQueryNumber;
		pBuf[p1].nQueryNumber = pBuf[p2].nQueryNumber;
		pBuf[p2].nQueryNumber = tmp1;

		StringBuffer_t *tmp2 = pBuf[p1].pBuf;
		pBuf[p1].pBuf = pBuf[p2].pBuf;
		pBuf[p2].pBuf = tmp2;
	}
}

/*
* Sort the buffer by the nValues.
*/
void
SortBufInt(Expr_Val_t *pBuf, int n)
{
	int i, j, k;
	for (i = 0; i < n; ++i)
	{
		...
		for (j = i + 1; j < n; ++j)
		{
			if (pBuf[k].nValue > pBuf[j].nValue)
			{
				k = j;
			}
		}
...
/*
* Sort the buffer by the buffer values.
*/
void
SortBufStr(Expr_Val_t *pBuf, int n)
{
	int i, j, k;
	for (i = 0; i < n; ++i)
	{
		...
		for (j = i + 1; j < n; ++j)
		{
			if (strcmp(GetBuffer(pBuf[k].pBuf), GetBuffer(pBuf[j].pBuf)) > 0)
			{
				k = j;
			}
		}
