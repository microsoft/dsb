...
distItemCount(char *name)
{
	...
	if ((d = find_dist(name)) == NULL)
	{
		char msg[80];
		sprintf(msg, "Invalid distribution name '%s'", name);
		INTERNAL(msg);
		assert(d != NULL);
	}
	...
	// This finds the max value of the last row in the value set.
	// It assumes the range of values are consecutive integers from 1 .. max value.
	char_val = dist->strings + dist->value_sets[d->length - 1][d->v_width - 1];
...
{
	int j, k;
	printf("\tname: %s[length: %d] --> (v_width: %d, w_width: %d)\n",
		t->name,
		t->length,
		t->v_width,
		t->w_width);
	if (t->name_space)
		printf(" names_space: %d\n", t->name_space);
	printf("\t[dist] strings: %s, size %d\n",
		t->dist->strings, t->dist->size);
	// Print the information of each weight set.
	// Print the information of each weight set.
	for (j = 0; j < t->w_width; ++j) {
		// maximum seems to be NULL.
		if (t->length < 5) {
			printf("\t[dist] weight set %d, weights: [", j);
			for (k = 0; k < t->length; ++k) {
				printf(",%d", t->dist->weight_sets[j][k]);
			}
			printf("]\n");
		}
		else {
			printf("\t[dist] weight set %d, last weight %d\n",
				j, t->dist->weight_sets[j][t->length - 1]);
		}
	}
	printf("\n");
}
...
void
print_dist_index(distindex_t * t)
{
	int i;
	d_idx_t *pIndexEntry;
	...
	{
		pIndexEntry = t->pEntries + i;
		print_dist(pIndexEntry);
	}
}
...
/*
* Generate a number from a distribution given a permutation
*/
int
dist_op_permutation_with_count(void *dest, int op, char *d_name, int vset, int wset, int stream, int * permutation, int count)
{
	pPermutation = permutation;
	permutationCount = count;
	int ret = dist_op(dest, op, d_name, vset, wset, stream);
	pPermutation = NULL;
	permutationCount = 0;
	return ret;
}
...
*/
int
dist_op_permutation(void *dest, int op, char *d_name, int vset, int wset, int stream, int * permutation)
{
	pPermutation = permutation;
	int ret = dist_op(dest, op, d_name, vset, wset, stream);
	pPermutation = NULL;
...
dist_op(void *dest, int op, char *d_name, int vset, int wset, int stream)
{
	...
	{
		dt = vset - 1;
		if (wset == DIST_EXP_RANK_LOW || wset == DIST_EXP_RANK_HIGH)
		{
			// This is only intended to use for the distribution
			// where the value entry is categorical and the first column is the values.
			// Because the size of the permutation can be smaller than d->length, i.e., active_cities,
			// if a permutation count is given, use that instead.
			int count = d->length;
			if (pPermutation != NULL && permutationCount > 0) {
				count = permutationCount;
			}
			genrand_integer(&index, wset, 1,
				count, 0, stream);
			// If external permutation is provided, map the index
			// based on permutation.
			if (pPermutation != NULL)
			{
				mapPermutation(&index, pPermutation);
			}
			index -= 1; // shift to 0-based indexing scheme.
