...
static int * pWholesaleCostPermutation = NULL;
static int mWholesaleCostBucketCount;

static int * pMarkupPermutation = NULL;
static int mMarkupBucketCount;

static int * pDiscountPermutation = NULL;
static int mDiscountBucketCount;

void set_pricing_permutation(int nTabId, ds_pricing_t * pPricing,
	int * wholesaleCostPermutation, int wholesaleCostBucketCount,
	int * markupPermutation, int markupBucketCount,
	int * discountPermutation, int discountBucketCount)
{
	pWholesaleCostPermutation = wholesaleCostPermutation;
	mWholesaleCostBucketCount = wholesaleCostBucketCount;

	pMarkupPermutation = markupPermutation;
	mMarkupBucketCount = markupBucketCount;

	pDiscountPermutation = discountPermutation;
	mDiscountBucketCount = discountBucketCount;

	set_pricing(nTabId, pPricing);

	pWholesaleCostPermutation = NULL;
	mWholesaleCostBucketCount = 0;

	pMarkupPermutation = NULL;
	mMarkupBucketCount = 0;

	pDiscountPermutation = NULL;
	mDiscountBucketCount = 0;
}
/*
...
void set_pricing(int nTabId, ds_pricing_t *pPricing)
{
	...
	case S_WLIN_PRICING:
		...
		// Add skews to data
		genrand_decimal_permutation(&pPricing->wholesale_cost, DIST_EXP_RANK_LOW, &dWholesaleMin, &dWholesaleMax, NULL, nTabId,
			pWholesaleCostPermutation, mWholesaleCostBucketCount);
		...
		// Add skews to data
		genrand_decimal_permutation(&dMarkup, DIST_EXP_RANK_LOW, &dMarkupMin, &dMarkupMax, NULL, nTabId,
			pMarkupPermutation, mMarkupBucketCount);
		...
		// Add skews to data
		genrand_decimal_permutation(&dDiscount, DIST_EXP_RANK_LOW, &dDiscountMin, &dDiscountMax, NULL, nTabId,
			pDiscountPermutation, mDiscountBucketCount);
