...
ds_key_t * pCdemoPermutation;

struct ATTRIBUTE_KEY_MAP * pHdemoMap;
ds_key_t * pHdemoPermutation;
...
mk_w_customer (void * row, ds_key_t index)
{
   ...
   tdef *pT = getSimpleTdefsByNumber(CUSTOMER);
	...
	if (!bInit)
	{
		...
		pBirthYearPermutation = makePermutation(NULL, nMax, C_BIRTH_DAY);
		pBirthMonthPermutation = makePermutation(NULL, 12, C_BIRTH_DAY);
		pBirthDayPermutation = makePermutation(NULL, 31, C_BIRTH_DAY);
		...
		pHdemoMap = deserializeAttributeKeyMap("household_demographics");
		pHdemoPermutation = makeKeyPermutation(NULL, pHdemoMap->attributeCount, C_CURRENT_HDEMO_SK);
	...
	*/
	...
	// Add skews to data
	pick_distribution_permutation(&r->c_birth_country, "countries", 1, DIST_EXP_RANK_LOW, C_BIRTH_COUNTRY, pCountryPermutation);
	...
	genrandSingleTableOneSidedCorrelatedJointDistribution(attribute_id,
		pCustomerAddressStateMap->attributeCount,
		pCdemoMap, pCdemoPermutation,
		DIST_EXP_RANK_LOW, 0.01, C_CURRENT_CDEMO_SK, &r->c_current_cdemo_sk);

	// Add joint distribution between state and household demographics.
	genrandSingleTableOneSidedCorrelatedJointDistribution(attribute_id,
		pCustomerAddressStateMap->attributeCount,
		pHdemoMap, pHdemoPermutation,
		DIST_EXP_RANK_LOW, 0.1, C_CURRENT_HDEMO_SK, &r->c_current_hdemo_sk);
