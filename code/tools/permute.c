...
#include "genrand.h"
 ...
 /*
 * Generate the range based on the number of buckets and the bucket ID.
 */
void
...
*/
void
mapPermutation(int * dest, int* pPermutation)
{
	int idx = *dest - 1; // Shift the index to 0 based.
	*dest = pPermutation[idx] + 1; // Shift the value to 1 based.
}

/*
* Map a key typed permutation
*/
void
mapKeyPermutation(ds_key_t * dest, ds_key_t* pPermutation)
{
	ds_key_t idx = *dest - 1; // Shift the index to 0 based.
	*dest = pPermutation[idx] + 1; // Shift the value to 1 based.
}
