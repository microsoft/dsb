...
// Copy the constants from w_customer_demographics.h
...
static int hdemo_column_count = 4;
static int hdemo_column_cards[4];
static int * hdemo_permutations[4];

static int cdemo_column_count = 8;
static int cdemo_column_cards[8];
static int * cdemo_permutations[8];

static ds_key_t * pPermutation = NULL;
...
static ds_key_t
household_demographics_join(ds_key_t *dest, int dist, int stream)
{
	static int bInit = 0;
	if (!bInit)
	{
		hdemo_column_cards[3] = distsize("income_band");
		hdemo_column_cards[2] = distsize("buy_potential");
		hdemo_column_cards[1] = distsize("dependent_count");
		hdemo_column_cards[0] = distsize("vehicle_count");
		for (int i = 0; i < hdemo_column_count; ++i)
		{
			hdemo_permutations[i] = makePermutation(NULL, hdemo_column_cards[i], stream);
		}

		bInit = 1;
	}
	return genrand_hierarchical_key(dest, dist, hdemo_column_count, hdemo_column_cards, stream, hdemo_permutations);
}

static ds_key_t
customer_demographics_join(ds_key_t *dest, int dist, int stream)
{
	static int bInit = 0;

	if (!bInit)
	{
		cdemo_column_cards[7] = distsize("gender");
		cdemo_column_cards[6] = distsize("marital_status");
		cdemo_column_cards[5] = distsize("education");
		cdemo_column_cards[4] = distsize("purchase_band");
		cdemo_column_cards[3] = distsize("credit_rating");
		cdemo_column_cards[2] = CD_MAX_CHILDREN;
		cdemo_column_cards[1] = CD_MAX_EMPLOYED;
		cdemo_column_cards[0] = CD_MAX_COLLEGE;
		for (int i = 0; i < cdemo_column_count; ++i)
		{
...
ds_key_t
mk_join_permutation(int from_col, int to_tbl, ds_key_t join_count, ds_key_t * permutation)
{
	pPermutation = permutation;
	ds_key_t result = mk_join(from_col, to_tbl, join_count);
	pPermutation = NULL;
	return result;
}
...
mk_join(int from_col, int to_tbl, ds_key_t join_count)
{
	...
	case HOUSEHOLD_DEMOGRAPHICS:
		return household_demographics_join(&res, DIST_EXP_RANK_LOW, from_col);
	case CUSTOMER_DEMOGRAPHICS:
		return customer_demographics_join(&res, DIST_EXP_RANK_LOW, from_col);
	default:
	/*
	 * all TYPE2 tables (i.e., history keeping dimensions) need a special join algorithm
	 */
		if (pTdef->flags & FL_TYPE_2)
			return(scd_join_permutation(nTableIndex, from_col, join_count, pPermutation));
		...
		// Add skews to data
		genrand_key(&res, DIST_EXP_RANK_LOW, (ds_key_t)1, get_rowcount(nTableIndex), (ds_key_t)0, from_col);
		if (pPermutation != NULL)
		{
			mapKeyPermutation(&res, pPermutation);
		}
		break;
	}
