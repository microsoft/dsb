...
static int employeeNumberBucket;
static int employeeNumberBucketCount;
static int * pEmployeeNumberPermutation;
static int * pMarketIdPermutation;
static int * pFloorSpacePermutation;
static int floorSpaceBucket;
static int floorSpaceBucketCount;
...
if (!bInit)
	{
		...
		makeBucket(&employeeNumberBucket, &employeeNumberBucketCount, 20, nMax - nMin + 1);
		pEmployeeNumberPermutation = makePermutation(NULL, employeeNumberBucketCount, W_STORE_TYPE);

		pMarketIdPermutation = makePermutation(NULL, 10, W_STORE_MARKET_ID);

		dist_member(&nMin, "store_type", nStoreType, 4);
		dist_member(&nMax, "store_type", nStoreType, 5),
		makeBucket(&floorSpaceBucket, &floorSpaceBucketCount, 20, nMax - nMin + 1);
		pFloorSpacePermutation = makePermutation(NULL, floorSpaceBucketCount, W_STORE_FLOOR_SPACE);
  ...
  */
    ...
    */
	...
	// Add skews to data
	int tmp;
	genrand_integer(&tmp, DIST_EXP_RANK_LOW, 1, employeeNumberBucketCount, 0, W_STORE_EMPLOYEES);
	mapPermutation(&tmp, pEmployeeNumberPermutation);
	makeRangeFromBucket(&rangeMin, &rangeMax, employeeNumberBucket, tmp, nMin, nMax);

	genrand_integer(&r->employees, DIST_UNIFORM, rangeMin, rangeMax, 0, W_STORE_EMPLOYEES);
	...
	// Add skews to data
	genrand_integer(&r->floor_space, DIST_EXP_RANK_LOW, 1, floorSpaceBucketCount, 0, W_STORE_FLOOR_SPACE);
	mapPermutation(&r->floor_space, pFloorSpacePermutation);
	makeRangeFromBucket(&rangeMin, &rangeMax, floorSpaceBucket, r->floor_space, nMin, nMax);

	genrand_integer(&r->floor_space, DIST_UNIFORM, rangeMin, rangeMax, 0, W_STORE_FLOOR_SPACE);
	...
	// Add skews to data
	r->market_id = genrand_integer(NULL, DIST_EXP_RANK_LOW, 1, 10, 0, W_STORE_MARKET_ID);
	mapPermutation(&r->market_id, pMarketIdPermutation);
