...
get_rowcount(int table)
{
	...
	{		
		...
		for (nTable=0; nTable <= MAX_TABLE; nTable++)
		{
			switch(nScale)
			{
			case 100000:
...
ds_key_t
dateScalingPermutation(int nTable, ds_key_t jDate, int * datePermutation)
{
	pDatePermutation = datePermutation;
	ds_key_t result = dateScaling(nTable, jDate);
	pDatePermutation = NULL;
	return result;
}
...
dateScaling(int nTable, ds_key_t jDate)
{
   ...
   static int daysMin, daysMax;
	...
	if (!bInit)
	{
		...
		printf("scaling date permutation: count %d, bucket %d, bucket count %d\n", totalDays, dateBucket, dateBucketCount);
	...
	*/
		/*static int prevBucket = 0;
		static int totalRows = 0;
		static int totalCount = 0;*/

		int days = dttoj(&Date);
		// Due to numerical errors, it can happen that the sum of the number of rows is less than the total number of rows.
		// In this case, the days date can go beyond the maximal date.
		// When this happens, setting the date to the maximal date and use the weight of the last bucket.
		if (days > daysMax)
		{
			days = daysMax;
			printf("[WARNING] dateScaling: date goes beyond the maximal date\n");
		}
		int bucketIdx = (days - daysMin) / dateBucket + 1; // The 1-based index of this date.
		makeRangeFromBucket(&rangeMin, &rangeMax, dateBucket, bucketIdx, daysMin, daysMax); // Range of this bucket.
		if (pDatePermutation != NULL)
		{
			mapPermutation(&bucketIdx, pDatePermutation);
		}
		double weight = get_exp_rank_weight(bucketIdx, dateBucketCount, DIST_EXP_LAMBDA_LOW) / (rangeMax - rangeMin); // Assume uniform distribution within a bucket.
		weight *= 1 + 0.03; // Add epsilon to avoid numerical inaccuracy that can lead to insufficient number of rows, which causes the date to go beyond the max date.
		kRowCount = (int)(kRowCount * weight);
		/*if (bucketIdx != prevBucket)
		{
			printf("dateScaling: prevBucketIdx %d, totalCount %d, totalRows %d, newBucketIdx %d, weight %.6f, rows %d\n", prevBucket, totalCount, totalRows, bucketIdx, weight, kRowCount);
