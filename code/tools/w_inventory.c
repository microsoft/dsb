...
static int * pInvQuantityPermutation;
static int invQuantityBucket;
static int invQuantityBucketCount;
...
mk_w_inventory(void *pDest, ds_key_t index)
{
   ...
   tdef *pTdef = getSimpleTdefsByNumber(INVENTORY);
	...
	if (!bInit)
	{
		...
		invQuantityBucketCount = 20; // Ideal number of buckets.
		invQuantityBucket = (INV_QUANTITY_MAX - INV_QUANTITY_MIN + invQuantityBucketCount) / invQuantityBucketCount; // Compute size of buckets.
		invQuantityBucketCount = (INV_QUANTITY_MAX - INV_QUANTITY_MIN + invQuantityBucket) / invQuantityBucket; // Update the number of buckets.
		pInvQuantityPermutation = makePermutation(NULL, invQuantityBucketCount, INV_QUANTITY_ON_HAND);
	...
	// Add skews to data
	int tmp;
	genrand_integer(&tmp, DIST_EXP_RANK_LOW,
