...
int *	genrand_gaussian_dist(int n, double sigma, double center, int stream);
double *	genrand_gaussian_probability(int n, double sigma);
int		genrand_integer(int *dest, int dist, int min, int max, int mean, int stream);
double * remap_exp_cdf(int * permutation, long long count, double lambda);
double	get_exp_cdf(long long rank, long long count, double lamda);
double	get_exp_rank_weight(long long rank, long long count, double lambda);
long long genrand_exp_rank(long long min, long long max, double lambda, int stream);
int		genrand_decimal(decimal_t *dest, int dist, decimal_t *min, decimal_t *max, decimal_t *mean, int stream);
int		genrand_decimal_permutation(decimal_t *dest, int dist, decimal_t *min, decimal_t *max, decimal_t *mean, int stream, int * permutation, int bucketCount);
int		genrand_date(date_t *dest, int dist, date_t *min, date_t *max, date_t *mean, int stream);
int		genrand_date_permutation(date_t *dest, int dist, date_t *min, date_t *max, date_t *mean, int stream, int * yearPermutation, int * monthPermutation, int * dayPermutation);
ds_key_t	genrand_key(ds_key_t *dest, int dist, ds_key_t min, ds_key_t max, ds_key_t mean, int stream);
ds_key_t	genrand_hierarchical_key(ds_key_t * dest, int dist, int count, int * sizes, int stream, int ** permutations);
